...
Every mod file copy goes through the [resource governor](resources.md), and under the default
...
the buffer, the pause and the parallelism: see [Presets](resources.md#presets).
...
| One per kind of work (governor) | under Balanced: hashing ≤ 4 (about half your cores), deploy 2, extract and compress the size of the global pool | Each kind gets its own, sized by the preset, so a hash job and a deploy never fight over one pool. BLAKE3 alone is fast enough to eat every core — see [Integrity & hashing](integrity-hashing.md) |
