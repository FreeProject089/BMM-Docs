...
!!! warning "A deploy in the worker process can be cancelled, not paused"
    ...
    ([Performance](performance.md#getting-the-heavy-work-out-of-the-window)). The app holds the
...
let the task's end put everything back. See [Scheduling & automation](../features/scheduler.md#how-hard-bmm-works).
...
The routes, tools and commands are listed in the [API reference](../reference/api.md), the
[MCP server reference](../reference/mcp.md) and the [CLI reference](../reference/cli.md).
