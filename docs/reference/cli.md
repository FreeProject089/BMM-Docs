...
The executable that serves BMM's [MCP tools](mcp.md) is also a **command-line tool**. Same
...
!!! tip "Three ways in, one core"
    ...
    The [local HTTP API](api.md) is for plugins and scripts, [MCP](mcp.md) is for AI clients,
...
| Command | Arguments | What it does |
|---|---|---|
| `call` | `<method>`\*, `<path>`\*, `<body>` | Call the running app's [local API](api.md) directly — `call GET /api/status`. Everything the CLI does not wrap is reachable this way |
...
| `bmms-reference` | — | The whole [BMMScript](../features/bmmscript-reference.md) vocabulary as JSON: actions and their parameters, conditions, value sources, loop sources, keywords, permissions, script engines |
...
That is the same arrangement the [MCP reference](mcp.md) has, and for the same reason: a list
...
- [MCP server reference](mcp.md) — the same binary's other half, and the 69 tools it exposes
- [Local API &amp; deeplinks](api.md) — what `call` is calling
- [BMMScript reference](../features/bmmscript-reference.md) — the language `bmms-compile` reads
- [Action reference](actions.md) — what `actions` lists
