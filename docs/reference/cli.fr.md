...
[Ouvrir dans Aide &amp; autres](bmm://docs/open?article=api-reference){ .md-button }
...
!!! tip "Trois portes, un seul cœur"
    ...
    L'[API HTTP locale](api.fr.md) est pour les plugins et les scripts, [MCP](mcp.fr.md) est
...
| Commande | Arguments | Ce que ça fait |
|---|---|---|
| `call` | `<method>`\*, `<path>`\*, `<body>` | Appelle directement l'[API locale](api.fr.md) de l'app en cours — `call GET /api/status`. Tout ce que la CLI n'enveloppe pas est atteignable ainsi |
...
| `bmms-reference` | — | Tout le vocabulaire [BMMScript](../features/bmmscript-reference.fr.md) en JSON : actions et leurs paramètres, conditions, sources de valeurs, sources de boucle, mots-clés, permissions, moteurs de script |
